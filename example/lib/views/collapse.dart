...
class CollapsePageState extends State {
  ...
  Widget build(BuildContext context) {
    ...
    ['1', '2', '3'].forEach((item) {
      options
          .add(WeCollapseItem(title: Text('选项$item'), child: Text('内容$item')));
    });
    ...
    return Sample('Collapse',
        ...
        child: Column(children: <Widget>[
          TextTitle('默认'),
          WeCollapse(children: options),
          TextTitle('设置默认展示'),
          WeCollapse(defaultActive: ['1'], children: options),
          TextTitle('卡片方式'),
          WeCollapse(card: true, children: options),
          TextTitle('手风琴模式'),
          WeCollapse(accordion: true, children: options),
          TextTitle('onChange'),
          WeCollapse(
              children: options,
